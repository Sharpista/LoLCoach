...
- [ ] **T-DB-3** Revisar `sql/001_runtime_functions.sql` do pacote `hermes-agent-runtime` contra o schema real: transações, advisory lock, grants, status permitidos, payloads e ausência de `SECURITY DEFINER`.
...
- [ ] **T-DB-6** Testar concorrência em ambiente seguro: duas tentativas de claim para a mesma issue, uma deve ganhar e a outra deve falhar sem run órfão.
- [ ] **T-DB-7** Testar recovery de lock expirado e tentativa tardia de finish do worker antigo.
...
- [ ] **T-ORCH-8** Adicionar testes unitários para classificação de labels, bloqueios por política, conflito de claim, gates de qualidade e recovery.
...
- [ ] **T-BE-2** Implementar a tradução Kanban -> `ExecutionResult`: `tests_status`, `review_status`, `commit_sha`, `pull_request_url`, `railway_deployment_id` quando aplicáveis.
...
- [ ] **T-BE-5** Testar adapter com store fake e dispatcher fake cobrindo sucesso, falha, bloqueio por gate ausente e perda de lock.
...
- `T-QA-*` só começa depois de banco, orquestrador, adapter e runbook suficientes existirem.
